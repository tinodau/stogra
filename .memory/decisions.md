...
## ADR-007: ESLint 9 Flat Config

**Date:** 2025-02-18 07:00 UTC
**Status:** Accepted

**Context:**
ESLint 9 requires the new flat config format (`eslint.config.js`). The old `.eslintrc.json` format is no longer supported.

**Decision:**
Migrate to `eslint.config.js` using `typescript-eslint` flat config presets.

**Rationale:**

- ESLint 9+ requires flat config
- `typescript-eslint` provides recommended presets for flat config
- Simpler configuration with explicit ignores
